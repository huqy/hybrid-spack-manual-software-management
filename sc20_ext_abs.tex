...
\begin{abstract}
In this work we present a new approach for managing the scientific packages in an HPC cluster environment, particularly suitable in a research university environment that has a diverse user base.
...
The goal is trying to lessen the HPC operational team's burden when installing and maintaining a large number of software packages to support a broadband spectrum of university research. 
We propose a hybridizing management method that can harness both the power of a software management framework and the flexibility of in-house, or "manual", installation sources, and can present an coherent view to the end users. Spack~\cite{gamblin:15} framework was used as an example in this work, but our approach is expected to be applicable to other framework tools. The manipulation of environment module files and the typical workflow are illustrated, followed by discussions.
\end{abstract}

\begin{IEEEkeywords}
software management, modules, Spack, HPC 
\end{IEEEkeywords}
...
Having to install and maintain a large number of software packages to support the broadband spectrum of university research motivates the use of a software installation framework, such as EasyBuild\cite{geimer:14} or Spack\cite{gamblin:15}, to automate the process and thus lessens the burden on the HPC operational team. 
However, challenges arise when the framework has to be integrated with in-house, or "manual", package installation practice already existed. In practice, the in-house installation component is required because not all packages can be satisfactorily managed within a software management framework.

%%{\color{gray}(4) our approach and values/contributions of our work}
In this work, we propose a practical approach for HPC cluster operational team to manage software packages by hybridizing the framework and manual-based installation methods. 
From a software design point of view, the central idea is to add a layer above the existing software management framework, where a workflow is introduced to manage applications built either by the framework or by manual installation to enable flexible customization, complying to the existing structure of the HPC cluster environment.
In this work, we adopt Spack as the package management framework. The proposed approach is applicable to using other software management frameworks. We consider the deployment of the hybrid approach on UCLA Hoffman2 Cluster where an in-house package installation structure already exists.
It should be noted that the file system locations of framework-installed and manually-installed packages are kept separate by design, so it will allow future changes of the HPC environment, including the possibility of adopting another package management framework.
A main contribution of this work is a noble way to integrate, or hybridize, two software package management systems and present a coherent view to end users for ease of use.
%manipulate and re-organize the environment modules and offer a practical, automated and script-able workflow to build and maintain the packages, and to provide a coherent view to end users.
...
Adding a flexible management layer on top of framework relies on the exploration of the detailed module file structure generated by the framework.
...
Specifically, Spack installation process always produces many duplication of installed packages/libs. Assigning a unique hash code for every version of all packages introduces the new confusions for general-level presentation to users.
At the same time, hierarchical structure generated from Spack is fixed. In some situations, the convoluted file structure defined from Spack may not be preferable and it has no way to customize it. 
...
In the SQLite database file (\verb|apps.db|), there is currently only one table (\verb|apps|) in the database. The schema of the table can be reviewed by \verb|.schema apps| inside the SQLite shell. The detailed scripts can be publicly accessed in the gitlab repo\cite{gitlabrepo}.


\section{Practical Workflows} \label{sec_workflow}

The practical workflow process to hybridize the Spack-installed and manually-installed packages can be built based on the proposed structures. Inparticular we use the JSON file and SQLite file defined in Section \ref{subsec_modulefile_processing} to coordinate both ways of installation. Figure \ref{fig:spack_h2_hybrid_flow} below shows one possibility of workflow of installing a package, from the installation request to displaying the module.
...
In this work we proposed an approach to hybridize the software management framework and the traditional manual package management. The design adds a layer on top of the standard package management framework, in order to take advantage of the powerful framework, the flexibility of the traditional manual management and the desired the HPC user interface, via the use of modulefiles constructed to prevent loading incomptabible packages. With the definition of a customizable module file structure in the HPC cluster environment, the workflow is able to process, filter and integrate the module files from two independent sources (i.e. framework-based and manual-based) to any target modulefile presentation. 
