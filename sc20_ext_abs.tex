...
We propose a hybridizing management method that can harness both the power of a software management framework and the flexibility of in-house, or ``manual'', installation sources, and can present a coherent view to the end users. Spack~\cite{gamblin:15} framework was used as an example in this work, but our approach is expected to be applicable to other framework tools. The manipulation of environment module files and the typical workflow are illustrated, followed by discussions.
\end{abstract}
