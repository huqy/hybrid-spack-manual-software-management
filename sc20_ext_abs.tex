...
Having to install and maintain a large number of software packages to support the broadband spectrum of university research motivates the use of a software installation framework, such as EasyBuild\cite{geimer:14} or Spack\cite{gamblin:15}, to automate the process and thus lessen the burden on the HPC operational team. 
However, challenges arise when the framework has to be integrated with in-house package installation practice already existed. In practice, the in-house installation component is required because not all packages can be satisfactorily managed within a software management framework.
...
Specifically, Spack's installation process can create multiple duplications of installed packages and libraries. Presenting all versions of the Spack-built software packages, with embeded unique hash codes in the package's display names, at once can confuse the end users.
In addition, the predefined hierarchical structure create by Spack is quite rigid and incompatible for our use case. There seems to be no easy way to customize it. 
...
Based on our own experience, also motivated by the operation of other large-scale HPC clusters (e.g. TACC Stampede), we propose that the module files are organized in a simple structure as shown in Fig. \ref{fig:h2_new_hier}, where the MPI level is nested in the compiler level. 
